...
template <bool or_node>
void ParallelDfPn::dfpn_inner(Position& n, const int thpn, const int thdn/*, bool inc_flag*/, const uint16_t maxDepth, int64_t& searchedNode) {
	...
	if (entry.num_searched == 0) {
		if (or_node) {
			...
			for (const auto& ml : move_picker) {
				...
				//// debug
				//if (14526697373827801542LL == root_key_debug) {
				...
				//		std::cout
				//			<< "[mate][or_node][before entry2][" << entry.num_searched << ", " << entry.depth << "] (" << tmpEntry.pn << ", " << tmpEntry.dn << ") "
				//			<< tmpEntry.depth << ", " << tmpEntry.hand.value() << std::endl;
				//	}
				//}

				//// debug
				//std::cout << "[mate][or_node][entry2][" << entry.num_searched << ", " << entry.depth << "] start" << std::endl;
				auto& entry2 = transposition_table.LookUp<false>(n);
				//std::cout << "[mate][or_node][entry2][" << entry.num_searched << ", " << entry.depth << "] done" << std::endl;
				...
				//	if (16721636401499889413LL >> 32 == tmpEntry.hash_high) {
				//		std::cout
				//			<< "[mate][or_node][after entry2][" << entry.num_searched << ", " << entry.depth << "] (" << tmpEntry.pn << ", " << tmpEntry.dn << ") "
				//			<< tmpEntry.depth << ", " << tmpEntry.hand.value() << std::endl;
				//	}
				//}
				...
				if (move_picker2.size() == 0) {
					...
					//// debug
					////std::cout << "[mate][or_node][root is matroot is matee][" << entry.num_searched << ", " << entry.depth << "]" << std::endl;
					//Hand tmpHand(4196361);
					//auto&& tmpEntry = transposition_table.LookUp(16721636401499889413, tmpHand, 6);
					//if (16721636401499889413LL >> 32 == tmpEntry.hash_high) {
					//	// hand = 0b10000000000100000001001, つまりpawn 9, Knight 2, bishop 1 なので恐らく例の局面
					//	std::cout
					//		<< "[mate][or_node][root is mate][" << entry.num_searched << ", " << entry.depth << "] (" << tmpEntry.pn << ", " << tmpEntry.dn << ") "
					//		<< tmpEntry.depth << ", " << tmpEntry.hand.value() << ", " << root_key_debug << std::endl;
					//}
				...
				if (n.gamePly() + 2 > maxDepth) {
					...
					//	if (16721636401499889413LL >> 32 == tmpEntry.hash_high) {
					//		std::cout
					//			<< "[mate][or_node][if(n.gamePly() + 2 > maxDepth)][" << entry.num_searched << ", " << entry.depth << "] (" << tmpEntry.pn << ", " << tmpEntry.dn << ") "
					//			<< tmpEntry.depth << ", " << tmpEntry.hand.value() << std::endl;
					//		std::cout
					//			<< "ply = " << n.gamePly()
					//			<< " maxDepth = " << maxDepth
				...
				for (const auto& move : move_picker2)
				{
					...
					if (n.mateMoveIn1Ply<mmin1ply_Additional>()) {
						//std::cout << "[mate][or_node][entry1][" << entry.num_searched << ", " << entry.depth << "] start" << std::endl;
			...
			for (const auto& move : move_picker)
			{
				...
				if (const Move move = n.mateMoveIn1Ply<mmin1ply_Additional>()) {
					//std::cout << "[mate][and_node][entry1][" << entry.num_searched << ", " << entry.depth << "] start" << std::endl;
					auto& entry1 = transposition_table.LookUp<true>(n);
					//std::cout << "[mate][and_node][entry1][" << entry.num_searched << ", " << entry.depth << "] done" << std::endl;
					entry1.pn = 0;
					entry1.dn = kInfinitePnDn + 2;
	...
	while (searchedNode < maxSearchNode && !stop) {
		...
		if (or_node) {
			...
			for (const auto& move : move_picker) {
				auto& ttkey = ttkeys[&move - move_picker.begin()];
				//std::cout << "[if(or_node)][" << entry.num_searched << ", " << entry.depth << "] start" << std::endl;
				const auto& child_entry = transposition_table.LookUpDirect(*ttkey.entries, ttkey.hash_high, ttkey.hand, n.gamePly() + 1);
				//std::cout << "[if(or_node)][" << entry.num_searched << ", " << entry.depth << "] done" << std::endl;
			...
			for (const auto& move : move_picker) {
				auto& ttkey = ttkeys[&move - move_picker.begin()];
				//std::cout << "[if(and_node)][" << entry.num_searched << ", " << entry.depth << "] start" << std::endl;
				const auto& child_entry = transposition_table.LookUpDirect(*ttkey.entries, ttkey.hash_high, ttkey.hand, n.gamePly() + 1);
				//std::cout << "[if(and_node)][" << entry.num_searched << ", " << entry.depth << "] done" << std::endl;
				if (all_mate) {
					if (child_entry.pn == 0) {
						// NOTE: 証明駒の和集合を求める。
...
// 詰将棋探索のエントリポイント
bool ParallelDfPn::dfpn(Position& r) {
	// キャッシュの世代を進める
...
// 詰将棋探索のエントリポイント
bool ParallelDfPn::dfpn_andnode(Position& r) {
	// 自玉に王手がかかっていること
