...
bool Position::pseudoLegalMoveIsLegal(const Move move, const Bitboard& pinned) const {
    ...
    // NOTE
    //    : 玉の移動ではないと言い切れない場合は、移動する駒の種類を確認。
    //      玉なら以下の処理。
    if (!FROMMUSTNOTKING && pieceToPieceType(piece(from)) == King) {
        const Color them = oppositeColor(us);

        // NOTE
        //     : 玉の移動先(move.to()) に敵方からの何らかの利きがあれば(attackersToIsAny==true)、それは非合法手。
...
// とあるように、遠隔駒の利きがある箇所には移動しないように指し手を生成した場合にのみ有効。
bool Position::pseudoLegalMoveIsEvasion(const Move move, const Bitboard& pinned) const {
    ...
    if (move.pieceTypeFrom() == King) {
        // NOTE
        //     : まず、attackersTo は、各コマに対して以下の操作をしている。
        //       簡単のため、桂馬についての処理を考える。
        //       move.to() の位置に、仮に敵の桂馬が存在したとした場合の利きを求めて、
        //       その利きに自分の桂馬が存在すればその駒の位置を1 としている。
        //     : 以下の処理では、"敵の歩" "自玉" "敵の飛車" 
        //       という配置の時、"敵の歩" の位置にもし敵の飛車 が存在した場合の利きを求め、
        //       その利きの中に今の"敵の飛車"の位置が入っていれば良いわけだが、
        //       doMove() せずにそれを行うので、"敵の歩" の位置に仮に存在するとした飛車の利きは、
        //       "自玉" の手前で止まる(つまり右側に利きは一切ない)。
        //       なので、その利きのbb と飛車のbb のand は0 となってしまう。
        //       -> 本当は"敵の歩" の位置に移動しても"敵の飛車" からの利きから逃れられないのに、
        //          逃れれたかのように錯覚してしまう。
        //       -> 遠隔の利きがある時は、以下のようにdoMove() を省くことは出来ない。
        // 遠隔駒で王手されたとき、王手している遠隔駒の利きには移動しないように指し手を生成している。
    ...
    if (move.isDrop()) {
        ...
        if (inCheck()) {
            if (ptFrom == King) {
                ...
                // NOTE
                //     : 恐らく、玉の元居た位置に立ってるbit を0 にしてるのじゃないかな？
                //       そうしないと、pseudoLegalMoveIsEvasion() のコメントにあるように、
                //       遠隔の駒の利きがある箇所に逃げた時に、doMove() してないので、
                //       自玉が壁となって利きが無いと錯覚してしまう。
                occ.clearBit(from);
...
void Position::doMove(const Move move, StateInfo& newSt, const CheckInfo& ci, const bool moveIsCheck) {
    ...
    // NOTE
    //     : この二つのkey (boardKey, handKey) の和がこの局面のzobrist_hach() となる。
    //       c.f. StateInfo::Key()
    //     : doMove 時に毎度更新する。
    ...
    // debug
    const std::string move_usi = move.toUSI();
...
// NOTE: 符号。なんかトリッキーな書き方してるね。まぁスタイリッシュではあるけど。
// sgn関数
...
// NOTE: sq1に対してsq2の升の延長上にある、sq2の次の升を得る
...
void initMate1Ply() {
    ...
    // NOTE
    //     : SQUARE_NB ってのは、SquareNum のことだね恐らく
    //     : s2 から更に、"s1からs2の方向"に一つずれた升をnext_sq としてる
    // NextSquareの初期化
    ...
    for (Square s1 = SQ11; s1 < SquareNum; ++s1)
        for (Square s2 = SQ11; s2 < SquareNum; ++s2) {
            Square next_sq = SquareNum;    // これが無いときの初期値。None的な。
...
template <Color US, bool Additional> Move Position::mateMoveIn1Ply() {
    ...
    // FIXME
    //     : なんかassert 引っかかる。
    //       王手が掛かっているか否かを調べるassert だとしたら、これには引っかからんはずやけどな
    //       board.mateMove() は、inCheck() によって場合分けしてるから。。。
...
silver_drop_end:
    ...
    if (Additional) {
        ...
        if (!themHand.exceptPawnExists()) {    // NOTE: 敵が、歩を除く任意の駒が持ち駒にない。

            // NOTE
            //     : ksq は敵(them) の玉の位置
            //     : なので、敵の駒がある個所以外が、敵の玉が移動できる箇所
            // 玉の8近傍の移動可能箇所の列挙
            ...
            // NOTE
            //     : 完全には分かってないけど文脈から察するに、
            //       これが、"敵の駒がない & 手番側からの利きが無い箇所"、即ち"敵の玉が移動可能(退避可能)な箇所"
            Bitboard escape_bb = bb_king_movable & ~aak; // 利きがない場所が退路の候補

            // NOTE: 敵の退避可能なマス目(箇所)の合計
            ...
            while (bb2) {
                ...
                // NOTE
                //     : nextSquare() はコメント見る限りどうやら、ksq の、one 側の延長線上にある次の升を返すらしい？
                //     : 次ってのは、ksq の一つ次なのか？それともone の一つ次なのか？
                //       -> one の次。つまりこの場合はksq から2つ隣(oneはすぐ隣なので一つ隣)
                // toの地点が盤外
                ...
                if (dr & Effect8::DIRECTIONS_DIAG) {
                    ...
                    else if (!ourHand.exists<HRook>()) {    // 飛車を持っていない
                        goto NEXT2;
                    }
                }
                ...
                // oneに駒の移動合いができない
                if (can_piece_capture(*this, Them, one, dcBB_betweenIsThem, occupiedBB())) {
                    goto NEXT2;
                }

                // toに打つ駒が取れない
                if (can_piece_capture(*this, Them, to, dcBB_betweenIsThem, occupiedBB())) {
                    goto NEXT2;
                }
...
void Position::toHuffmanCodedPos(u8* data) const {
	std::fill(data, data + 32, 0);    // NOTE: HuffmanCodedPos::clear() に相当
    BitStream bs(data);
    // 手番 (1bit)
    bs.putBit(turn());
    ...
    // 盤上の駒
    // NOTE
    //     : 各升になんの駒(当然emptyも駒扱い)があるかをSQ11から順に最後まで見ていく。
    //       
    for (Square sq = SQ11; sq < SquareNum; ++sq) {
        Piece pc = piece(sq);
...
// dlshogi では直接HuffmanCodedPos を返す関数を使っていたで、
// Position::toHuffmanCodedPos(u8* data) のラッパーとして、同じ機能を持つ関数を追加
HuffmanCodedPos Position::toHuffmanCodedPos() const {
    HuffmanCodedPos result;
    toHuffmanCodedPos(result.data);
    return result;
}
...
RepetitionType Position::isDraw(const int checkMaxPly) const {
    ...
    //std::cout << "e: " << e << ", st_->pliesFromNull: " << st_->pliesFromNull << std::endl;
    ...
    if (i <= e) {
        ...
        //std::cout << (st_ == nullptr ? "nullptr" : "not nullptr") << ", "
        //    << (st_->previous == nullptr ? "nullptr" : "not nullptr") << ", "
        //    << (st_->previous->previous == nullptr ? "nullptr" : "not nullptr") << ", " << std::endl;
...
// https://qiita.com/hiro4669/items/44144f0f9739a8aa1285
Position& Position::operator = (const Position& pos) {
    //std::cout << "info: Position::operator = () start" << std::endl;
...
// NOTE: set() で呼ばれる。
void Position::clear() {
    // NOTE: 本当に、あらゆるものが0 初期化される、って理解でええんかな？
...
// NOTE
//     : 手番c の駒の内、位置sq に対して利きを通している駒のbb.
//     : 何度も書いてるが、例えば歩なら、
//       (attacksFrom<Pawn  >(opposite, sq          ) & bbOf(Pawn  )
//       は、sq に存在すると仮定した敵の歩の利きの中に存在する自分の歩のみが、sq に利きを通せる。
