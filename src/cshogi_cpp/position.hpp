...
struct StateInfo {
    ...
    // https://yaneuraou.yaneu.com/2015/12/16/%E9%80%A3%E8%BC%89%E3%82%84%E3%81%AD%E3%81%86%E3%82%89%E7%8E%8Bmini%E3%81%A7%E9%81%8A%E3%81%BC%E3%81%86%EF%BC%817%E6%97%A5%E7%9B%AE/
...
// https://blog.csdn.net/weixin_42369301/article/details/126473447
// これがないと、map, unordered_map が使えない。まぁ当たり前だね。
template <>
...
public:
    ...
    // NOTE
    //     : notOccupiedBB()、即ちemptyBB()
    //     : 恐らく、right が7 * 9, left が2 * 9 升分を表現してる奴なんだろうな。
    //       ほんでもって、盤外の箇所が全部1 になっても良いならemptyBB()よりこっちの方がわずかに速いはず、ってことなんだろうな。
    // emptyBB() よりもわずかに速いはず。
    ...
    template <PieceType PT> Bitboard attacksFrom(const Square sq) const {
        ...
        // NOTE
        //     : 角、飛車、玉、馬、竜 は駒の利きが対称で、
        //       同じ位置なら先手の駒と後手の駒 とで利きに差がない。
        // Color は何でも良い。
    ...
    // NOTE
    //     : 以下追加
    StateInfo* get_state() const {
        return st_;
    }

    StateInfo* get_start_state() {
        return &startState_;
    }

    void print_addr() const {
        std::cout << "info: print_addr() start" << std::endl;
        std::cout << "info: piece_ = " << &piece_[0] << ", " << &piece_[SquareNum - 1] << std::endl;
        std::cout << "info: kingSquare_ = " << &kingSquare_[0] << ", " << &kingSquare_[ColorNum - 1] << std::endl;
        std::cout << "info: hand_ = " << &hand_[0] << ", " << &hand_[ColorNum - 1] << std::endl;
        std::cout << "info: turn_ = " << &turn_ << std::endl;
        std::cout << "info: startState_ = " << &startState_ << std::endl;
        std::cout << "info: st_ = " << st_ << std::endl;
        std::cout << "info: gamePly_ = " << &gamePly_ << std::endl;
    }

    void print_si_history() const {
        int i = 0;
        StateInfo* crrt_si = st_;
        std::cout << "[crrtKey] " << getBoardKey() << "," << getHandKey() << std::endl;
        do {
            std::cout << "[" << i << "] " << crrt_si->boardKey << "," << crrt_si->handKey << std::endl;
            crrt_si = crrt_si->previous;
            ++i;
        } while(crrt_si != nullptr);
    }
...
private:
    ...
    // NOTE
    //     : FindPinned == true なら、自玉と敵  の遠隔駒の間を考える。
    //     : FindPinned == true なら、敵玉と自分の遠隔駒の間を考える。
    // turn() 側が
    ...
    template <bool FindPinned, bool BetweenIsUs> Bitboard hiddenCheckers() const {
        ...
        while (pinners) {
            ...
            if (between
                && between.isOneBit<false>()    // NOTE: 間にある駒が複数あるなら、その駒全部動かないと王手にならんでの、pin されてるとは言わん。
                && between.andIsAny(bbOf(BetweenIsUs ? us : them)))    // NOTE: 間にある駒の内少なくとも一つが(BetweenIsUs ? 自分の駒 : 敵の駒) である。
            {
                result |= between;    // pinされてる or pinしている 追加
            }
        }
