...
public:
    Hand() {}
    explicit Hand(const u32& v) : value_(v) {}
    explicit Hand(u32&& v) : value_(std::move(v)) {}
    Hand(const Hand& hand) : value_(hand.value_) {}
    Hand(Hand&& hand) noexcept : value_(std::move(hand.value_)) {}
    ...
    //       https://yohhoy.hatenadiary.jp/entry/20160417/p1
    //       ref-qualify (reference qualifyer と言うらしい)
    //           Note: unlike cv-qualification, ref-qualification
    //           does not change the properties of the this pointer:
    //           within an rvalue ref-qualified function,
    //           *this remains an lvalue expression.
    //       訳
    //           cv修飾子と違い、参照修飾子はthis の属性を変更しない:
    //           右辺値参照修飾された関数内でも、*this は左辺値式のままである。
    //     : https://qiita.com/rinse_/items/0139dd8598f2d58e668c
    //       const 修飾子は、メンバ関数に暗黙の内に渡されているthis ポインタをconst とする。
    //     : 以下は使いたかったので使ってみた。
    Hand& operator=(const Hand& hand) & { value_ = hand.value_; return *this; }
    Hand& operator=(Hand&& hand) & noexcept { value_ = std::move(hand.value_); return *this; }
    Hand& operator=(const Hand& hand) && = delete;
    Hand& operator=(Hand&& hand) && noexcept = delete;
    ...
    void plusN(const HandPiece handPiece, const int n) { value_ += n * HandPieceOne[handPiece]; }
    void minusN(const HandPiece handPiece, const int n) { value_ -= n * HandPieceOne[handPiece]; }
    ...
    // TODO: const u32& value にすべきでは？もっと言うと、move 版も作るべきかな。
	// 証明駒用メソッド
    void set(const u32& value) { value_ = value; }
    void set(u32&& value) { value_ = std::move(value); }
