...
enum PieceType {
    ...
    PieceTypeNum,    // NOTE: Occupiedも含めて計15個？

    GoldHorseDragon // 単にtemnplate引数として使用
};
extern const std::string PieceTypeToStrTable[PieceTypeNum];
OverloadEnumOperators(PieceType);

// bitobard 上等で、"先後・成り/不成・駒" を区別。
enum Piece {
...
extern const std::string HandPieceToStrTable[HandPieceNum];
...
// MY: (@20230807 追加)
inline std::string HandPieceToStr(const HandPiece hp) {
    return HandPieceToStrTable[hp];
}
inline std::string PieceTypeToStr(const PieceType pt) {
    return PieceTypeToStrTable[pt];
}
