...
namespace Effect8
{
    ...
    // YaneuraOu bitboard.cpp より
    inline void init() {
        // ------------------------------------------------------------
        //        Bitboard関係のテーブルの初期化
        // ------------------------------------------------------------

        // NOTE: sqww_tableは、to_sqww() に必要っぽい。
        // 1) SquareWithWallテーブルの初期化。

        for (auto sq = SQ11; sq < SquareNum; ++sq) {
            // NOTE: 各SQW_xx は、SQWW_11 からの相対移動で計算。
        ...
        for (auto sq1 = SQ11; sq1 < SquareNum; ++sq1) {
            ...
            {
                // NOTE
                //     : delta は、SQWW表現における、dir 方向に一つ動く際の変化。
                //       -> 以下のfor文は、sq1 の一つ隣から始めて、盤外に到達するまでの間、dir 方向に一つずつ移動し続ける
