			...
			if (MT != NonCaptureMinusPro) {
				...
				if (toOn123BB) {
					...
					FOREACH_BB(toOn123BB, to, {
					        ...
					        //const auto retMove = makePromoteMove<MT>(Pawn, from, to, pos);
							//(*moveList++).move = retMove;
							//std::cout << "[retMove:" << retMove.toUSI() << "]" << std::endl;
							(*moveList++).move = makePromoteMove<MT>(Pawn, from, to, pos);
							// NOTE
							//     : Apery の名残か知らんけど、NonEvasion だと無条件で2, 3段目への不成まで追加してたので、
							//       コメントアウトしておいた(他の所も同様にコメントアウト)。なんか理由あるんかね？
							if (/*MT == NonEvasion ||*/ ALL) {
								const Rank TRank1 = (US == Black ? Rank1 : Rank9);
								if (makeRank(to) != TRank1) {
									//const auto retMove2 = makeNonPromoteMove<MT>(Pawn, from, to, pos);
									//(*moveList++).move = retMove2;
									//std::cout << "[retMove2:" << retMove2.toUSI() << "]" << std::endl;
...
template <MoveType MT>
ExtMove* generateMoves(ExtMove* moveList, const Position& pos) {
	const Color pos_turn = pos.turn();
	if (pos_turn == Black) {
		ExtMove* retVal = GenerateMoves<MT, Black>()(moveList, pos);
		return retVal;
	}
	else if (pos_turn == White) {
		ExtMove* retVal = GenerateMoves<MT, White>()(moveList, pos);
		return retVal;
	}
	else {
		sync_cout << "info string Errro! pos_turn == ColorNum...?" << sync_endl;
		exit(1);
	}
}
#endif
